...
export default class BotGraph {
    ...
    generate = (dataObject:any) => new Promise<Attachment>(async (resolve, reject) => {
        ...
        const day = 86400000;
        const margin = 25;
        const days = timestamp => parseInt(((Date.now() - timestamp) / day).toFixed(0))
        ...
        bar.append("text")
            ...
            .attr("y", (d,i) => yScale(i)+(0.65*yScale.bandwidth()))
            .attr("font-size", "12")
            .text((d,i) => `${d.key.toUpperCase()}`)
        graph.append("g")
            .call(xAxis);
        graph.append("g")
            .call(yAxis);
        ...
        const pngBuffer = await sharp(inputBuffer, { density: 600 })
