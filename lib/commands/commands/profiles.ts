...
const verifyCode = async (
	...
	const continueVerifying = async verificationCode => {
		...
		if (oldData) {
			const isThisAccountRegistered = oldData[
				'accounts'
			].find(account => account.id === playerId);
			const account = {
...
const updateRankRoles = async (
	...
	if (roleToAdd)
		msg.member.addRole(roleToAdd).catch(err => log.WARN(err));
};
...
export const profile = async (msg: Discord.Message): Promise<void> => {
	...
	if (mentions.length === 0 && args.length !== 0) {
		msg.channel.send(
			createEmbed(
				`:information_source:  Unoptimal command use`,
				[
					{
						title: '___',
						content:
	...
	const user: Discord.User =
		mentions.length === 0
			...
			: msg.guild.members.find(
					member => member.id === mentions[0].id,
			  ).user;
	...
	if (!isBotUser(user)) {
		if (
			!userData ||
			!userData['membership'] ||
			!userData['membership'].find(
				s => s.serverId === msg.guild.id,
			)
		) {
...
export const description = async (msg: Discord.Message): Promise<void> => {
	...
	if (userData?.punished) {
		msg.channel.send(
			createEmbed(
				`❌ You are banned from writing own descriptions`,
				[
					{
						title: '___',
	...
	if (description.length > 1024) {
		...
		msg.channel.send(
			createEmbed(
				`:information_source: Your description is too long`,
				[
					{
						title: '___',
...
export const update = async (msg: Discord.Message): Promise<void> => {
	...
	if (!member) {
		msg.channel.send(
			createEmbed(
				`:information_source: You didn't register yet`,
				[
					{
						title: '___',
	...
	if (lastUpdated < timeoutUpdate) {
		const embed = new Discord.RichEmbed()
			...
			.setTitle(
				`:information_source: Profile recently updated`,
			)
			.setColor('0xFDC000');
		embed.addField(
			'___',
			`Wait ${toMMSS(
				timeoutUpdate - lastUpdated,
	...
	const updateAccounts = async (index: number) => {
		...
		updatedAcc.mastery = mastery.lastPlayed
			? mastery
			: updatedAcc.mastery;
...
export const topmembers = async (msg: Discord.Message): Promise<void> => {
	...
	const counts = guildMembers
		.filter(user =>
			msg.guild.members.find(m => m.id === user.discordId),
		)
		.map(user => {
			const membership = user.membership.find(
	...
	sorted.map((member, index) => {
		if (index < count && sorted[index]) {
			const justifiedPosition = justifyToLeft(
				(index + 1).toString(),
				2,
			);
			...
			const username = msg.guild.members.find(
				m => m.id === member.id,
			).user.username;
...
export const register = async (msg: Discord.Message): Promise<void> => {
	...
	if (
		oldData &&
		oldData.accounts &&
		oldData.accounts.length >= maxAccounts
	) {
		msg.channel.send(
			createEmbed(
				`❌ You registered maximum amount of accounts`,
				[
					{
						title: '___',
			...
			sentEmbed
				...
				.then(collected => {
					...
					if (c && c.name === '✅')
						...
						msg.author.send(
							createEmbed(
								`:information_source: Profile registering aborted`,
								[
									{
										title:
											'___',
...
export const unregister = async (msg: Discord.Message): Promise<void> => {
	...
	if (!oldData || !oldData.accounts) {
		msg.channel.send(
			createEmbed(
				`:information_source: Provided account isn't registered`,
				[
					{
						title: '___',
